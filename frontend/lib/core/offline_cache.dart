...
class OfflineCache {
  ...
  static Map<String, dynamic> _bootstrapPayload() {
    return {
      ...
      'serverTime': Hive.isBoxOpen('cacheBox')
          ? (Hive.box('cacheBox').get('lastSyncAt') ??
  ...
  static List<Map<String, dynamic>> _pendingApprovals() {
    ...
    for (final row in _boxMaps('attendance')) {
      final status =
      (row['approval_status'] ?? row['status'] ?? '').toString().toUpperCase();
      if (status == 'SUBMITTED' || status == 'PENDING') {
        out.add({
          ...
          'payload': row,
          'created_at':
          row['created_at'] ?? row['att_date'] ?? row['attendance_date'],
        });
      }
    }
    ...
    for (final boxName in ['invoices', 'collections', 'expenses']) {
      for (final row in _boxMaps(boxName)) {
        final approvalStatus = (row['approval_status'] ?? '')
            .toString()
            .toUpperCase();
  ...
  static Map<String, dynamic> forPath(String path) {
    ...
    if (p == '/api/v1/users') {
      ...
      return {
        'users': _filterUsers(rows, q['q']),
        'items': _filterUsers(rows, q['q']),
      };
    }

    if (p == '/api/v1/geo/divisions') {
      final rows = _cacheMaps('divisions');
      return {'ok': true, 'divisions': rows, 'items': rows};
    }

    if (p == '/api/v1/geo/districts') {
      final divisionId = q['division_id'];
      final rows = _cacheMaps('districts');
      final filtered = (divisionId == null || divisionId.isEmpty)
          ? rows
          : rows
          .where((e) => '${e['division_id'] ?? ''}' == divisionId)
          .toList();
      return {'ok': true, 'districts': filtered, 'items': filtered};
    }

    if (p == '/api/v1/geo/zones') {
      final rows = _cacheMaps('zones');
      return {'ok': true, 'zones': rows, 'items': rows};
    }

    if (p == '/api/v1/geo/areas') {
      final zoneId = q['zone_id'];
      final rows = _cacheMaps('areas');
      final filtered = (zoneId == null || zoneId.isEmpty)
          ? rows
          : rows.where((e) => '${e['zone_id'] ?? ''}' == zoneId).toList();
      return {'ok': true, 'areas': filtered, 'items': filtered};
    }
    ...
    if (p == '/api/v1/geo/territories' || p == '/api/v1/territories') {
      final areaId = q['area_id'];
      final districtId = q['district_id'];

      final rows = _boxMaps('territories');
      final filtered = rows.where((e) {
        final okArea = areaId == null || areaId.isEmpty
            ? true
            : '${e['area_id'] ?? ''}' == areaId;
        final okDistrict = districtId == null || districtId.isEmpty
            ? true
            : '${e['district_id'] ?? ''}' == districtId;
        return okArea && okDistrict;
      }).toList();

      return {'territories': filtered, 'items': filtered};
    }
